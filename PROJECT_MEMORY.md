...
- DOCX manuscript QA now checks package-level style/layout/reference-field signals such as missing `styles.xml`, missing page margins, missing page size settings, incomplete page size dimensions, broken header/footer or embedded-image target references, missing footnote/endnote targets, broken hyperlink relationships, internal hyperlinks that point to missing bookmarks, undefined paragraph styles, and missing Word citation/bibliography fields including `w:fldSimple` simple-field instructions.
