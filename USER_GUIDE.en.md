...
Currently it can read `.docx` text and inspect several package-level Word signals: missing `styles.xml`, missing page margins or page size settings, undefined paragraph styles, missing citation/reference fields, missing bibliography fields, images or drawing objects without alternative text, and leftover tracked changes or comments. It still does not render Word pages or replace final template review in Word.
