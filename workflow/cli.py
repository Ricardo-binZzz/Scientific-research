...
def build_parser() -> argparse.ArgumentParser:
    ...
    run_command_parser = sim_subparsers.add_parser("run-command", help="run an installed solver CLI command and capture its log")
    run_command_parser.add_argument("root_dir", help="working directory for the external command")
    run_command_parser.add_argument("--log", required=True, help="log file path to write stdout/stderr")
    run_command_parser.add_argument("external_command", nargs=argparse.REMAINDER, help="command to run after --")
    ...
    discover_parser = library_subparsers.add_parser("discover", help="search Crossref open metadata")
    discover_parser.add_argument("root_dir", help="library root directory")
    discover_parser.add_argument("--query", required=True, help="search query for open metadata")
    discover_parser.add_argument("--limit", type=int, default=10, help="maximum Crossref records to fetch")
    discover_parser.add_argument("--merge", action="store_true", help="merge discovered records into library-index.json")
    download_pdf_parser = library_subparsers.add_parser("download-pdf", help="download a direct/open PDF URL")
    download_pdf_parser.add_argument("root_dir", help="directory where the PDF should be saved")
    download_pdf_parser.add_argument("url", help="direct PDF URL")
    download_pdf_parser.add_argument("--filename", default="", help="optional local PDF file name")
    ...
    from_data_parser.add_argument("--show-legend", choices=["true", "false"], default="true")
    from_data_parser.add_argument("--show-grid", choices=["true", "false"], default="true")
    from_data_parser.add_argument("--palette", default="default", choices=["default", "colorblind", "mono", "journal"])
    from_data_parser.add_argument("--title-font-size", type=int, default=18)
    from_data_parser.add_argument("--label-font-size", type=int, default=15)
    from_data_parser.add_argument("--tick-font-size", type=int, default=14)
    from_data_parser.add_argument("--line-width", type=float, default=2.0)
    from_data_parser.add_argument("--tick-count", type=int, default=5)
