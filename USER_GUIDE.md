...
“扫描可用文件”会列出课题目录里常见的输入文件，例如 `simulation` 里的 CSV/JSON、`manuscript` 里的 `.md`、`.txt`、`.docx`，以及已经生成的图文件。扫描完成后，网页会自动把第一份可用仿真数据填到“仿真数据”和“生成图”的数据路径里，把第一份稿件填到“稿件检查”路径里，把文献 CSV 填到导入路径里，并把图表输出目录填成当前课题的 `figures` 文件夹。你仍然可以手动改这些路径。

如果你想快速判断文献库质量，可以在网页里点击“文献洞察”。它会把文献总数、已有摘要数量、缺失 PDF/笔记数量、高引用候选、关键词排行和数据库来源集中显示出来，比完整文献地图更适合快速扫一眼。
...
如果只想快速看文献库是否够用，不需要生成完整写作素材包，可以在网页里点击“文献洞察”。它会优先显示摘要覆盖、高引用文献、关键词排行和数据库来源，适合作为写综述前的快速检查。
