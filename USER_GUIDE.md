...
- 检查缺 PDF。
- 检查缺笔记。
- 查看文献洞察，包括摘要覆盖、高引用文献、关键词排行和数据库来源。
- 添加文献。
- 生成论文摘要卡。
- 生成文献检索记录。
- 预览仿真 CSV/JSON 数据。
...
网页现在会记住上次填写的课题目录。右侧“运行结果”区域可以复制结果、下载当前结果为 `.md` 文件，也可以清空后继续操作。点击任意操作后，结果区会进入运行中状态；如果你改了路径或参数，可以点“重跑：某个操作名”直接重新执行刚才那个操作。不能使用的按钮会显示为灰色禁用状态。每次操作成功后，“运行结果”标题下方会显示最近成功的操作名和时间，并保留最近几次成功操作的小标签，方便你确认刚刚完成的是哪一步、前面连续跑过哪些步骤。

如果某个操作没有成功，右侧结果区上方会显示“操作需要处理”提示卡，先告诉你最可能该检查什么，例如项目根目录、文件路径或必填字段。提示卡里也会有“去对应区域”按钮，点击后会展开并跳到最可能需要修改的功能区，并短暂高亮该区域。下方仍会保留原始错误文本，方便复制给别人排查。

网页会在发送前先检查明显缺失的内容。比如没有填写项目根目录，或者导入 CSV、预览仿真数据、生成图、检查稿件时没有填写对应文件路径，网页会直接提示你补哪个字段，并自动打开、高亮、聚焦到对应输入框，不会等后端报错。生成 SVG 图时还会提前检查输出目录和文件名。

网页中带“必填”标记的字段，是当前操作最容易漏填、但运行时必须用到的内容。第一次使用时优先把这些字段填完整。

点击“流程状态”时，结果区上方会显示“任务向导”：包括下一步建议、每个步骤是否就绪、当前缺口数量。每个步骤卡片里有“去处理”按钮，点击后会展开并跳到对应功能区。下方仍保留原始 Markdown 报告，方便复制或下载。新手可以先看任务向导，再决定下一步点哪个功能。

点击“项目总览”时，结果区上方会显示“项目总览”卡片：把文献条目、笔记文件、图表包、仿真导出和稿件数量放在一起，并给出下一步建议。它适合每次打开课题后先看一眼，判断当前最缺什么。

点击“创建课题”成功后，结果区上方会显示“课题已创建”卡片：直接给出新项目路径，并提供“填入项目根目录”按钮。新手可以点这个按钮后继续点“流程状态”或“扫描可用文件”。

点击“项目体检”时，结果区上方会显示“体检概览”：把文献、仿真、稿件、项目资产分成健康卡片，直接标出哪些模块需要处理。完整体检报告仍然保留在下方。

点击“检查稿件”时，结果区上方会显示“稿件检查概览”：把问题总数、警告/提醒数量、引用问题、图表问题和结构问题先做成卡片，并列出最需要优先处理的前几条问题。下方仍保留完整 Markdown 报告，方便复制或下载。

点击“预览数据”“汇总数值范围”或“校验数据”时，结果区上方会显示“仿真数据概览”：先显示数据行数、识别列数或校验状态、非数字列、缺口类型，再显示样例行或数值范围。新手可以先看上方卡片判断数据能不能继续画图，再看下方完整报告。

点击“生成 SVG 图”后，结果区上方会显示“图形生成结果”：列出生成的 `.svg` 和 `.json` 路径，并汇总图类型、尺寸、坐标列、配色、坐标范围、图例和网格设置。`.svg` 是可放进论文的图，`.json` 是复现用的参数记录。

如果你不知道先填什么，点击“加载示例课题”，网页会把项目目录切换到：
...
“流程状态”会按步骤告诉你当前课题哪里已经准备好，哪里还缺材料，并给出下一步建议。建议每次开始写作前先点一次“流程状态”或“项目体检”。

“扫描可用文件”会列出课题目录里常见的输入文件，例如 `simulation` 里的 CSV/JSON、`manuscript` 里的 `.md`、`.txt`、`.docx`，以及已经生成的图文件。扫描完成后，网页会自动把第一份可用仿真数据填到“仿真数据”和“生成图”的数据路径里，把第一份稿件填到“稿件检查”路径里，把文献 CSV 填到导入路径里，并把图表输出目录填成当前课题的 `figures` 文件夹。结果区上方还会显示“路径助手”卡片，你可以点“填入”把扫描到的路径重新填到对应表单。你仍然可以手动改这些路径。

如果你想快速判断文献库质量，可以在网页里点击“文献洞察”。它会把文献总数、已有摘要数量、缺失 PDF/笔记数量、高引用候选、关键词排行和数据库来源集中显示出来，并在结果区上方生成可视化卡片和列表。原始 Markdown 报告仍然保留在下方，方便复制或下载。

网页里的“添加文献”“导入 CSV 文献元数据”“文献搜索”“近期文献”“按来源查找”完成后，右侧会先显示文献操作结果卡片。它会显示当前条目数或命中数、查询类型、前几条题名，并提醒后续可以继续检查缺 PDF 和缺笔记。

网页里的“文献库统计”“检查缺 PDF”“检查缺笔记”完成后，右侧会先显示“文献资产概览”。它会把文献总数、缺 PDF、缺笔记、已找到资产和缺失项分成卡片，方便你直接决定下一步补哪些 PDF 或摘要卡。
...
保存完成后，网页右侧会显示“报告已保存”卡片，里面有报告类型、保存路径和这个报告适合做什么。新手可以先打开保存路径里的 `.md` 文件，再复制到论文草稿或继续整理。

网页里生成“写作素材包”“写作看板”“文献对比表”“文献地图”“追踪清单”时，右侧会先显示规划报告概览卡片。它会列出报告区块数量、可用条目数量、建议用途和后续动作，让你先判断这份报告该怎么用，再阅读下方完整 Markdown。
