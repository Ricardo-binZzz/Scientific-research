...
“扫描可用文件”会列出课题目录里常见的输入文件，例如 `simulation` 里的 CSV/JSON、`manuscript` 里的 `.md`、`.txt`、`.docx`，以及已经生成的图文件。这个功能适合在你忘记文件名时先查一下，再把路径填到对应输入框。
...
如果你用网页生成图，“图类型”可以直接选择趋势图、柱状图、误差棒图、热力图或等值线图。误差棒图需要填写“误差列”；热力图和等值线图需要把 `Y 列` 填成 y 坐标列，并填写“数值列”。网页里还可以填写图宽和图高，单位是 `mm`；不填时默认是 `180 x 120 mm`。如果想固定坐标轴范围，可以填写 `X 最小值`、`X 最大值`、`Y 最小值`、`Y 最大值`；不填则自动按数据范围计算。
