...
- `.docx` 图片或绘图对象是否引用了缺失的嵌入图片文件，是否缺少替代文本，或者内部超链接是否指向不存在的书签。
