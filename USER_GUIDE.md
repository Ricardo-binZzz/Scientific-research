...
网页现在会记住上次填写的课题目录。右侧“运行结果”区域可以复制结果、下载当前结果为 `.md` 文件，也可以清空后继续操作。点击任意操作后，结果区会进入运行中状态；每次操作成功后，“运行结果”标题下方会显示最近成功的操作名和时间，并保留最近几次成功操作的小标签，方便你确认刚刚完成的是哪一步、前面连续跑过哪些步骤。
