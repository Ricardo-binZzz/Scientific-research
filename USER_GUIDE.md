...
第一版聚焦“项目仪表盘 + 报告中心”，不在手机上编辑文献、上传仿真数据、配置图表或修改稿件。报告中心可以查看项目体检报告、写作材料包、写作看板、文献表、文献地图和追踪清单；连接页会先检查服务地址、配对码和项目路径，确认项目能被电脑端服务读取后才进入仪表盘。电脑端配对响应会返回已授权项目列表，如果连接页没有手动填写项目路径，会优先尝试电脑端启动时指定的第一个 `--project-root`。如果手机端发现未连接或请求失败，仪表盘、运行页和报告页会显示“重新连接工作台”入口，回到连接页重新填写地址、配对码和项目路径。

小程序目录里包含 `miniprogram/project.config.json`，便于用微信开发者工具以游客 AppID 或测试模式打开 `miniprogram/` 做本地调试。配置里关闭了本地调试的域名校验，但正式发布仍需按微信平台要求配置 HTTPS 和合法域名。

微信小程序正式发布会受到 HTTPS 和合法域名限制；本地伴随服务 V1 先面向开发和小范围测试。
...
- `.docx` 是否使用了未定义的段落样式或字符样式，图片或绘图对象是否引用了缺失的嵌入图片文件，是否缺少替代文本，或者内部超链接是否指向不存在的书签。
- `.docx` 是否残留 Word 修订痕迹或批注标记，批注引用是否缺少对应的批注定义，提交前建议在 Word 里接受/拒绝修订并清理批注。
...
现在 `.docx` 除了读取正文文本，还会检查一部分 Word 包结构问题，例如缺少 `styles.xml`、页面边距或页面大小设置缺失、页面宽高属性不完整、页眉/页脚引用的部件缺失、嵌入图片目标文件缺失、脚注/尾注引用目标缺失、批注引用目标缺失、超链接关系断开或目标为空、内部超链接指向不存在的书签、正文使用了未定义段落样式或字符样式、没有检测到 Word 引用/参考文献域、参考文献章节存在但没有 bibliography 域、普通域 `w:fldSimple` 里的 bibliography 指令、复杂域 `w:fldChar` 起止标记不配平、图片或绘图对象缺少替代文本，以及残留修订痕迹或批注标记。它仍然不能替代 Word 模板审阅或 Zotero 字段人工核对；最终排版仍需在 Word 里确认。
