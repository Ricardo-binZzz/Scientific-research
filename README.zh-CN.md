...
这是一个早期的本地优先工作台，不是成熟的软件生态。当前版本适合试用、评估流程和做小范围贡献；项目已经包含示例项目、网页界面、CLI、CI、隐私扫描、引用元数据，以及覆盖主要工作流模块、网页 action 和 JavaScript 语法检查工具的 167 个 unittest。
...
- DOCX 检查会检查文本和包级信号，包括页面大小、图片替代文本元数据和修订/批注痕迹，但不渲染 Word 页面，也不完整校验学校模板格式。
- 等值线图输入必须是完整矩形网格。
