...
| 论文检查 | 检查 Markdown、纯文本和 DOCX 草稿中的引用、标题层级、图号、图注、表注、DOCX 样式/页面设置、图片目标和替代文本、修订/批注痕迹、批注目标、超链接锚点和 Word 参考文献域结构问题 |
...
这是一个早期的本地优先工作台，不是成熟的软件生态。当前版本适合试用、评估流程和做小范围贡献；项目已经包含示例项目、网页界面、CLI、CI、隐私扫描、引用元数据，以及覆盖主要工作流模块、网页 action 和 JavaScript 语法检查工具的 177 个 unittest。
...
- DOCX 检查会检查文本和包级信号，包括页面大小是否存在、页面宽高是否完整、页眉/页脚部件引用、嵌入图片目标、脚注/尾注目标、批注目标定义、超链接关系和内部书签锚点、Word 参考文献域、普通域和复杂域起止标记是否配平、图片替代文本元数据和修订/批注痕迹，但不渲染 Word 页面，也不完整校验学校模板格式。
- 等值线图输入必须是完整矩形网格。
