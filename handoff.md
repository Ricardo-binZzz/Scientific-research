...
- Release readiness checker on 2026-05-27: `tools/check_release_readiness.py` now provides a read-only pre-publish summary for release zip validity, smoke-test script availability, and local `v0.1.0` tag drift; release package checks require the checker, and README/user guides include the command. Focused verification: `python -m unittest tests.test_release_package -v`; full verification reached 214 tests.
